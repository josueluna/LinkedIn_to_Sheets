# Changelog

All notable changes to this project will be documented in this file.

---

## v0.1.7
### ✨ Features
- Added custom column mapping for LinkedIn fields
- Added sheet header preview inside column mapping
- Added persistent column mapping saved in local storage

### 🔄 Changes
- Improved duplicate detection to work with both default and custom column mappings
- Improved mapping workflow by reloading headers based on selected spreadsheet tab
- Prevented invalid duplicate-column mappings

### 🎨 UI Improvements
- Added column mapping settings view
- Refined column mapping layout and controls
- Added custom mapping status indicator in the destination section
- Aligned version label inline with author in footer for a cleaner layout

---

## v0.1.6
### ✨ Features
- Added toast notifications for user feedback
- Improved duplicate detection messaging when profile already exists

### 🎨 UI Improvements
- Refined feedback states and visual responses in popup

---

## v0.1.5
### ✨ Features
- Added Google Sheets tab selection support
- Improved spreadsheet selection flow

---

## v0.1.4
### ✨ Features
- Added dev/production manifest setup (`manifest.dev.json` and `manifest.prod.json`)
- Added GitHub Pages landing page (`docs/index.html`)

### 🔄 Changes
- Updated build scripts to support environment-based configuration
- Improved project structure for better scalability

### 🐛 Fixes
- Fixed feedback button behavior in extension popup

---

## v0.1.3
### ✨ Features
- Added Google Sheets template copy button
- Added privacy policy landing page

### 🔄 Changes
- Updated manifest with production OAuth configuration
- Minor README improvements

---

## v0.1.2
### 🚀 Initial Release
- Initial Chrome extension structure
- Core LinkedIn profile extraction functionality
- Integration with Google Sheets

---

## v0.1.1
### ✨ Features
- Base project setup with Vite, React, and TypeScript

---

## v0.1.0
### 🚀 Project Setup
- Initial project scaffolding# Changelog

All notable changes to this project will be documented in this file.

---

## v0.1.7
### 🎨 UI Improvements
- Aligned version label inline with author in footer for a cleaner layout

---

## v0.1.6
### ✨ Features
- Added toast notifications for user feedback
- Improved duplicate detection messaging when profile already exists

### 🎨 UI Improvements
- Refined feedback states and visual responses in popup

---

## v0.1.5
### ✨ Features
- Added Google Sheets tab selection support
- Improved spreadsheet selection flow

---

## v0.1.4
### ✨ Features
- Added dev/production manifest setup (`manifest.dev.json` and `manifest.prod.json`)
- Added GitHub Pages landing page (`docs/index.html`)

### 🔄 Changes
- Updated build scripts to support environment-based configuration
- Improved project structure for better scalability

### 🐛 Fixes
- Fixed feedback button behavior in extension popup

---

## v0.1.3
### ✨ Features
- Added Google Sheets template copy button
- Added privacy policy landing page

### 🔄 Changes
- Updated manifest with production OAuth configuration
- Minor README improvements

---

## v0.1.2
### 🚀 Initial Release
- Initial Chrome extension structure
- Core LinkedIn profile extraction functionality
- Integration with Google Sheets

---

## v0.1.1
### ✨ Features
- Base project setup with Vite, React, and TypeScript

---

## v0.1.0
### 🚀 Project Setup
- Initial project scaffolding
